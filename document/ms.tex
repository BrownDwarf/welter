...
\subsection{Single Component spectral fit to each spectral order}
...
We computed the median value and 5$^{th}$ and $95^{th}$ percentiles of a burned-in subset of the samples.  Overall these 26 sets of point estimates for $\vt_{\ast,m}$ show relatively good agreement, with exceptions.  We have little constraint on $\logg$ and $\Z$ since full-spectrum fitting tends to wash out the weak signal contained in line wings for spectra with a finite level of random noise and systematic error.  Meanwhile, strong metal lines can bias estimates of $\Z$ at the expense of estimates for $\teff$.  We get consistent estimates for $v_z \sim 15.6 \pm 0.6$ and $\vsini \sim 28.0 \pm 0.9$, with the exception of two spectral orders at $6920 <\lambda \;(\mathrm{nm})< 7057$ and $8473 < \lambda \;(\mathrm{nm}) < 8706$.  These two outlying spectral orders converged to unreasonably large values for $\vsini$ and $a_{\rm G}$, the amplitude of the Gaussian process covariance kernel, and one of these orders has an unusually small value for effective temperature, $\teff\sim3500$, comparable to the lower limit of the employed model grid.  Visual comparison of the observed spectrum for a range of fits for these orders shows that no single emulated spectrum could reasonably fit these discrepant spectral orders because there are several large spectral outliers.  The other spectral orders show effective temperature point estimates in the range $\teff=4000\pm130$ K.  Figure \ref{fig:SingleTeffvsOrder} displays the $\teff$ point estimates, with the 5$^{th}$ and $95^{th}$ percentile error bars, placed at the central wavelength of each of the 26 spectral orders for the subset of ESPaDoNs orders that received full-spectrum fitting.  Full spectrum fitting was attempted for some of the other available ESPaDoNs orders, but many of these orders did not achieve convergence in the MCMC chains.  


% Comparing the single-component results from Optical to Infrared
These spectra have comparable $\hat V$ of 12.90 and 12.83 for the ESPaDoNs and IGRINS spectra respectively.  


The effective temperature derived from the optical spectral orders shows some scatter in the range 3500 K - 4250, with a median   is about The derived effective temperature is on-average about 800 K cooler in the $K-$band than in the optical or short-wavelength end of $H-$band.
...
\subsection{Two-temperature photophere model fit to each spectral order}
