...
Figure XX shows the ASASSN X-band monitoring plotted from the interval X-Y.  Our IGRINS observation dates of Z and Q show that the spectra were probably acquired during a phase of XX.  The ESPaDoNs spectra were at a phase of Z, indicating...

We fit all the data in the interval X to Z in the following way:

\begin{figure*}
	\centering
	\includegraphics[width=0.95\textwidth]{figures/phase_folded_lightcurve.pdf}
	\caption{Phase of photometry at the time of IGRINS measurement}
	\label{fig:PhotPhase}
\end{figure*}

\section{Results}

\subsection{Result 11}
We expect that applying approach 11 (single component to all orders , \iancze) is a poor model for our multicomponent stellar data and would therefore yield a poor quality fit.  Nevertheless we experimented with a single temperature fit to $N_{ord}=X$ IGRINS spectral orders with X samples sampling Theta and Phi parameters in the blocked Gibbs sampler identically to \iancze.  We found relatively poor fit quality, with large typical residuals of X.  The poor fit is exemplified in the top panel of Figure X which shows the observed spectrum in the region of the X line.  The best fit model with theta = [x, y, z] does not achieve a good fit of the X line.  

\subsection{Result 12: Signal from deriving $\teff$ in each IGRINS order}

We chunked the IGRINS spectrum into $N_{ord}=$58 of its cleanest spectral orders and derived the stellar parameters $\vt_{\ast,m}$ independently in each $m^{\mathrm{th}}$ order.  We found 58 unique estimates for $\teff$ as a function of wavelength, which we compile in Figure \ref{fig:teffOrder}.  The figure shows a dramatic effect: \textbf{The derived effective temperature is on-average about 800 K cooler in the $K-$band than in the optical.}
...
\begin{figure*}
	\centering
	\includegraphics[width=0.95\textwidth]{lkca4_lores} 
	\vspace{-100mm}
...
\begin{figure*}
	\centering
	\includegraphics[width=0.95\textwidth]{tiovar} 
	\vspace{-100mm}
