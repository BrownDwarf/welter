...
\subsection{Rotational Modulation in the $V$-band lightcurve}\label{sec:rotSpot1}

The  $V$ brightness corresponds to a filling factor of the cool spot based on two temperatures fit from IGRINS spectra.  Although our analysis is consistent with a range of acceptable temperatures and filling factors, we adopt components of 2800 K covering 80\% of the visible stellar surface and 4200 K covering 20\% of the stellar surface at the time of the IGRINS spectrum, with an estimated $V_{rm est}=12.84$.  With these temperatures, the $V$-band emission is dominated by the hotter component.  The $V$-band brightness is therefore a good proxy for the visible surface area of the hot component.

Figure~\ref{fig:vband_spot} shows the recent ASAS-SN lightcurve, converted into a cool spot filling factor.  In these epochs, the brightest and coolest phase corresponds to cool component fill factors of $74\%$ and $86\%$, respectively.  Since 1992 the $V$ band photometry has been as bright as $12.3$ mag, corresponding to a cool component fill factor of $65\%$, and as faint as $13.2$ mag, or a cool component fill factor of $87\%$.  This drastic change in visible area of the hot spot (35\% to 13\%) is needed to explain the full $\sim 1$ mag range in brightness.


\subsection{The Spectral Energy Distribution}\label{sec:SED}

Figures~\ref{fig:sed}-\ref{fig:lores} compares synthetic spectra from the two-component photosphere to the SED and flux-calibrated spectra of LkCa 4.
The only free parameters are the luminosity and extinction, which are both scaled to match the spectrum.
The optical-IR SED is obtained from estimating photometry from \citet{grankin08} during the 2MASS epoch.  We also included mid-IR photometry from Spitzer/IRAC \citep{hartmann05}, without adjusting for epoch.  In the spectroscopic comparison, some minor discrepancies may be introduced because the spot coverage changed in the $\sim 5$ hrs between the optical and near-IR spectra.  The synthetic spectrum is obtained from the Phoenix models, as in \S 3.

These comparisons require extinction estimates.  The $B-V$ color from \citet{grankin08} has an excess $E(B-V)=0.15$ mag relative to empirical colors of a 4200 K main sequence star and a $0.27$ mag excess for a pre-main sequence star \citep{kenyon95,pecaut13}.  The main sequence colors are probably most appropriate in this analysis because they are more free of spots.  Under this assumption, the $B-V$ color excess leads to an extinction $A_V=0.45\pm0.15$ mag, where the uncertainty range  corresponds to colors of 4100--4300 K temperature photospheres.   This extinction is overestimated if cooler temperatures also contribute to optical emission in a multi-temperature photosphere.  More accurate extinction estimates from the full SED are challenging because they depend on the areal distribution of temperatures in the photosphere, which in this work is simplied to two temperatures.  J--K discussion.  Bizarre V--R color note...
...
\begin{figure}
 \centering
 \includegraphics[trim=3.1cm 13cm 2.3cm 3.6cm, clip=true, width=0.49\textwidth]{figures/phot1_paper}
...
\begin{figure*}
 \centering
 \includegraphics[trim=2.3cm 2.3cm 1.8cm 13.9cm, clip=true, width=0.75\textwidth]{figures/lkca4_lores_twopanel}
...
\begin{figure*}
 \centering
 \includegraphics[trim=1.6cm 12.6cm 2.2cm 2.4cm, clip=true, width=0.60\textwidth]{figures/vband_spot}
...
The TiO band depths in these spectra are measured from spectral indices listed in Table~\ref{tab:tio}.  The TiO indices are also combined into a single index by  calculating the number of standard deviations each point is from the median value of the index, and then averaging the standard deviation.  
