...
We took two approaches to characterizing the effective temperature response from a spotted-star spectrum.  First we measured a single unique effective temperature to each spectral order in the optical (\S \ref{sec:ESP_starfish}) and near-IR (\S \ref{sec:IGR_starfish}).  Second we fit a two-temperature mixture model (\S \ref{sec:methods}) to a near-IR echelle spectrum (\S \ref{sec:two_tempIGRINS}), which should show greater starspot flux density than optical.
