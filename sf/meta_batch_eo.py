...
mdirs = ['eo{:03d}'.format(m) for m in ms]
...
for mdir in mdirs:
    ...
    out_dir = "output/LkCa4_{}/run01/".format(mdir)
    print("os.chdir(out_dir)")
    print("Spawning a screen")
    screen_command = 'screen -d -m bash -c ' + '"cd /home/gully/GitHub/welter/sf/{}/output/LkCa4_{}/run01/'.format(mdir, mdir)+'; run.sh"' 
